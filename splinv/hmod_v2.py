...
class triaxialHalo(Cosmo):
    ...
    def __init__(self, ra, dec, redshift, mass, a_over_b, a_over_c, conc=None, rs=None, omega_m=Default_OmegaM):
        # Redshift and Geometry
        # ra dec
        ...
        self.Delta_e = 5 * self.Delta_vir * (self.a_over_b * self.a_over_c) ** (-0.75)  # eqn 6 OLS03

        self.m_c = 2 * np.log(np.sqrt(self.c) + np.sqrt(1 + self.c)) - 2 * np.sqrt(self.c / (1 + self.c))  # eqn 8 OLS03
...
class triaxialJS02(triaxialHalo):
    ...
    def __init__(self, ra, dec, redshift, mass, a_over_b, a_over_c, conc=None, rs=None, omega_m=Default_OmegaM):
        triaxialHalo.__init__(self, ra, dec, redshift, mass, a_over_b, a_over_c, conc=None, rs=None,
                              omega_m=Default_OmegaM)
    # def __DeltaSigma(self, x0):
    #     '''could introduce cutoff'''
    #     return 0
    ...
    def f(self, theta, phi):
        '''eqn 17 in OLS'''
        c2_a2 = (np.abs(self.a_over_c)) ** (-2)
        c2_b2 = (np.abs(self.a_over_c / self.a_over_b)) ** (-2)
        return np.sin(theta) ** 2 * (c2_a2 * np.cos(phi) ** 2 + c2_b2 * np.sin(phi)) + np.cos(theta)
    ...
    def h_ols(self,theta,phi,xp,yp):
        ...
        c2_a2 = (np.abs(self.a_over_c)) ** (-2)
        c2_b2 = (np.abs(self.a_over_c / self.a_over_b)) ** (-2)
        firstline = (c2_a2 * np.sin(phi)**2 + c2_b2*np.cos(phi)**2)*xp**2 + np.sin(2*phi)*np.cos(theta)*(c2_a2-c2_b2)*xp*yp
