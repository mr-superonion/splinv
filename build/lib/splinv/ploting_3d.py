...
def Gaussian_filter(x, mu, sigma, amplitude = 1):
    return 1/(sigma * np.sqrt(2 * np.pi)) * np.exp(-1/2*( (x - mu) / (sigma))**2)

import plotly
print(data1.shape)
import plotly.graph_objects as go
z, ra, dec = np.mgrid[0.05:2.5:19j, -10:10:96j,-10:10:96j]#np.mgrid[0.05:2.5:19j, -10:10:192j,-10:10:192j]
for i in range(19):
    # data2[i] =  data2[i] * (Gaussian_filter(Grid.zcgrid[i], 0.75, 0.05))
    # data3[i] = data3[i] * (Gaussian_filter(Grid.zcgrid[i], 1.75, 0.05))
    data1[i] = data1[i] * (Gaussian_filter(Grid.zcgrid[i], 1.25, 0.05))
...
fig = go.Figure(data = [halo1,halo2])'''
...
fig.update_layout(scene = dict(
                                    xaxis = dict(
                                         ...
                                         zerolinecolor="white",nticks = 5, tickvals = [0.25, 0.75, 1.25, 1.75, 2.25], ticktext = ['0.25', '0.75', '1.25', '1.75', '2.25']),
                                    yaxis = dict(
                                        backgroundcolor="rgba(0, 0, 0, 0)",
                                        gridcolor="lightslategrey",
                                        showbackground=False,
                                    ...
                                    zaxis = dict(
                                        ...
                                        gridcolor="lightslategrey",
                                        showbackground=False,
                                        zerolinecolor="white", nticks = 4,tickvals = [-7.5, -2.5, 2.5,7.5], ticktext = ['-15', '-5', '5', '15'])),xaxis_title = "Redshift",
                                    yaxis_title = "ra",
)
fig.update_layout(scene = dict(
