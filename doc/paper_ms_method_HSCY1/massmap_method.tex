...
$l^2$ chi-square term\footnote{The term chi-square is weighted by the inverse
of the diagonal covariance matrix of the error on the shear measurements
($\Sigma$).} measuring the difference between the prediction and the data,
while $C(x)$ is the regularization term measuring the deviation of the
coefficient estimation ($x$) from the prior assumption. The $l^p$ norm is
defined as
\begin{equation}
\norm{x}_p=\left(sum_{i} |x|^p_i\right)^{\frac{1}{p}}.
\end{equation}
...
direction is much larger than the halo scale. Therefore, we set the profile of
the NFW atoms in the line of sight direction as the Dirac delta function.
Moreover, we assume that the halos are sparsely distributed in the universe.
With the sparsity prior, the adaptive lasso regularization
\citep{AdaLASSO-Zou2006} is used to reconstruct the density field.  We find
that, in contrast to the lasso estimator that smears the structure along the
line of sight, the adaptive lasso can efficiently reduce the smearing effect.
...
We then introduce the dictionary used to model the foreground density maps in
...
\begin{figure}
 ...
 \caption{The average PDF of MLZ photo-$z$ PDFs for $10$ source redshift bins defined
         in Figure \ref{fig_bestpz}.
        }\label{fig_pdfpz}
\end{figure}
...
at the position of lens, $H_0$ is the Hubble constant, $\Omega_M$ is the matter
density parameter, $c$ is the speed of light, $\chi_{sl}$ is the comoving
distance between source and lens plane, and $a(\chi_l)$ is the scale parameter
at the lens position.
...
\end{cases}
\end{equation}
where $E(z)$ is the Hubble constant as a function of redshift, in units of $H_0$.
...
\end{equation}
Here we denote the physical shear distortion as $\gamma_L$, which is not the
final shear measurement since the final shear measurement is influenced by
systematic errors from observations. The systematic errors will be discussed in
Section \ref{subsec_method_Systematics}.
...
As the scales of halos are much smaller than the reachable redshift resolution,
we neglect the depth of halo on the line of sight direction and set the
profiles of the NFW atoms in the line of sight direction to one-dimensional
($1$-D) Dirac delta functions as suggested by
\citep{LSS-massMap-Glimpse3D-Leonard2014}. The multi-scale NFW atoms are
defined as
\begin{equation}
\begin{split}
...
different scale frames. We then write the basis atoms into the angular
...
\caption{The density map reconstructions with the lasso (left) and the adaptive
    ...
    are neglected in the simulation.  The input mass of halo is
...
negligible redshift uncertainties and photometric redshifts with redshift
...
the source galaxy redshift estimation are negligible. However, as demonstrated
...
\end{split}
\end{equation}
In this paper, we set $\beta=1\farcm5$.
...
of $1\arcmin$. The grids in the line of sight direction follow
...
N_\theta \times N_l \times N$, where $N$ is the number of NFW dictionary
frames. The corresponding pixelized elements of the forward transform matrix
$\mathbf{A}$ is denoted as $A_{\alpha\beta}$.
...
the inverse of the diagonal elements the noise covariance matrix is defined as
\begin{equation}
...
Figure \ref{fig_lassoVsadaLasso} shows the reconstructions of the mass map of a
single halo with halo mass $M_{200}=10^{15} ~h^{-1}M_{\odot}$ at redshift
$0.35$. In this paper, we set the non-linear over-density ($\Delta_{\rm{vir}}$)
to $200$, and use $M_{200}$ to denote the halo mass. The shear measurement
error and photo-$z$ uncertainty are not included in the simulation.  The left
panel of Figure \ref{fig_lassoVsadaLasso} is the reconstruction with the lasso
algorithm, which shows significant smearing along the line of sight.
...
(FISTA) to solve the lasso problem. Since the loss functions of the lasso and
the adaptive lasso only differ in their penalization terms, the FISTA is also
applicable to solve the adaptive lasso problem. In this paper, we apply the
FISTA to solve both the preliminary lasso estimation and the final adaptive
lasso estimation.
...
As summarized in Algorithm \ref{alg-1}, we first initialize the projection
